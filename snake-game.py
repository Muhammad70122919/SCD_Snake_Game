import turtle                   #for graphic
import random
import time

class SnakeGame:
    def __init__(self):
        self.screen = self.setup_screen()    #creates a window for drawing
        self.draw_border()
        self.score = 0
        self.delay = 0.1
        self.snake = self.create_snake()     #function
        self.fruit = self.create_fruit()      #function
        self.old_fruit = []
        self.scoring = turtle.Turtle()    # creates a turtle object.
        self.setup_scoring()              #function
        self.setup_keyboard_controls()     #function
    ...
    def draw_border(self):
        turtle.speed(5)
        turtle.pensize(4)     # Set the pen size to 4 units
        turtle.penup()         # Lift the pen up - turtle won't draw while moving
        turtle.goto(-310, 250)     # Move to a different location without drawing
        turtle.pendown()       # Put the pen down - turtle will draw while moving
        turtle.color('black')
        for _ in range(2):
            turtle.forward(600)
            turtle.right(90)
    ...
    def snake_move(self):  # Add this method
